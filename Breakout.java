...
public class Breakout extends GraphicsProgram {
	...
	public void run() {
		System.out.println("DESTROY ALL THE BRICKS");
		System.out.println("Cyan = 1 point, Green = 2 points, Yellow = 3 points, Orange = 4 points, Red = 5 points.");
	...
	private void drawBricks(double cx, double cy) {
		 ...
		 * loops, one loop for the rows and one for loop for the columns.
		 */
	...
	public void moveBall() {
		...
		// through the wall at that point
		...
		// so all that is left is the brick
		else if (collider != null && collider.getColor() != Color.MAGENTA && collider.getColor() != Color.BLACK
				&& collider.getHeight() != 15) {
			remove(collider);
			int addedPoints = 0;
			...
			vy = -vy;
			score = giveScore;
			GLabel points = new GLabel("+" + addedPoints, collider.getX() + 40, collider.getY() + 20);
			points.setFont("TimesNewRoman-30");
			points.setColor(collider.getColor());
			add(points);
			remove(displayScore);
			writeInfo();
			addMouseListeners();
			waitForMouseMoved(null);
			remove(points);
		}
		pause(DELAY);
	}

	private void waitForMouseMoved(MouseEvent m) {
	}
	...
	private void writeInfo() {
		GLabel title = new GLabel("DESTROY ALL THE BRICKS", 0, 30);
		title.setFont("TimesNewRoman-30");
		title.setColor(Color.magenta);
		title.sendToBack();
		add(title);
		instruction = new GLabel(
				"Cyan = 1 point, Green = 2 points, Yellow = 3 points, Orange = 4 points, Red = 5 points.", 1, 60);
		instruction.sendToBack();
		add(instruction);
		displayScore = new GLabel("Score = " + giveScore, 1000, 30);
		displayScore.setFont("TimesNewRoman-30");
		displayScore.setColor(Color.magenta);
		displayScore.sendToBack();
		add(displayScore);
	}

	private void printGameOver() {
		GLabel gameOver = new GLabel("Game Over " + giveScore + " point(s)", getWidth() / 2, getHeight() / 2);
		gameOver.move(-gameOver.getWidth() / 2, -gameOver.getHeight());
		gameOver.setColor(Color.RED);
		gameOver.setFont("TimesNewRoman-60");
		add(gameOver);
		waitForClick();
		removeAll();
		run();
	...
	private void printWinner() {
		GLabel Winner = new GLabel("Winner!!! " + giveScore + " point(s)!", getWidth() / 2, getHeight() / 2);
		Winner.move(-Winner.getWidth() / 2, -Winner.getHeight());
		Winner.setColor(Color.RED);
		Winner.setFont("TimesNewRoman-60");
		add(Winner);
		waitForClick();
		removeAll();
		run();
